// api/send-kimura.js
// 木村捺々恵さん専用メッセージ送信API（最重要・予定取得問題解決済み）
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const defaultMessage = `お疲れさまです、ななみさん！

パン作りが得意でいつもお疲れさまです。

【重要】
木村捺々恵さんの予定取得問題も完全に解決しました。
システム連携テスト完了です。
...
個別API機能が正常に動作しています。`;
    ...
    const result = await sendBotMessage(accessToken, kimuraInfo.email, messageText);
    
    return res.status(200).json({
      success: result.success,
      message: result.success ? 
        "✅ 木村捺々恵さんにメッセージ送信完了！（予定取得問題解決済み）" : 
        "❌ 木村捺々恵さんへの送信に失敗しました",
      data: {
        target: kimuraInfo,
        ...
        botName: "日向",
        apiType: "木村さん専用API（最重要・予定問題解決済み）",
        importance: "⭐ 最重要スタッフ"
      },
