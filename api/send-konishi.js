// api/send-konishi.js - 小西瞳さん専用API
...
export default async function handler(req, res) {
  ...
  try {
    const konishiInfo = { name: "小西瞳", email: "oh.19136@ohisamafarm", role: "職業指導員", nickname: "ひとみ", specialty: "トラクター運転・体力仕事" };
    const customMessage = req.query.message || req.body?.message;
    const defaultMessage = `お疲れさまです、ひとみさん！\n\n体力・運転技術でトラクター運転も可能で、\nいつも頼りにしています。\n\n小西瞳さん専用APIが正常に動作しています。\nClaude.aiシステムテスト中です。`;
    const messageText = customMessage || defaultMessage;
    const accessToken = await getAccessToken();
    if (!accessToken) throw new Error('認証失敗');
    const result = await sendBotMessage(accessToken, konishiInfo.email, messageText);
    return res.status(200).json({
      success: result.success,
      message: result.success ? "✅ 小西瞳さんにメッセージ送信完了！" : "❌ 送信失敗",
      data: { target: konishiInfo, messageType: customMessage ? "カスタムメッセージ" : "デフォルトメッセージ", deliveryStatus: result.success ? "配信成功" : "配信失敗", status: result.status, botName: "日向", apiType: "個別専用API" },
      timestamp: new Date().toISOString()
    });
  } catch (error) {
    return res.status(200).json({ success: false, error: error.message, target: "小西瞳さん", timestamp: new Date().toISOString() });
  }
}
