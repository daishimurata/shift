...
export default async function handler(req, res) {
  ...
  try {
    const nakaiInfo = { name: "中井理恵", email: "oh.88782@ohisamafarm", role: "職業指導員", nickname: "りえ", specialty: "手芸が得意" };
    const customMessage = req.query.message || req.body?.message;
    const defaultMessage = `お疲れさまです、りえさん！\n\n手芸が得意で、村田太志さんと最古参の付き合いで\nいつもお疲れさまです。\n\n中井理恵さん専用APIが正常に動作しています。\nシステム連携テストです。`;
    const messageText = customMessage || defaultMessage;
    const accessToken = await getAccessToken();
    if (!accessToken) throw new Error('認証失敗');
    const result = await sendBotMessage(accessToken, nakaiInfo.email, messageText);
    return res.status(200).json({
      success: result.success,
      message: result.success ? "✅ 中井理恵さんにメッセージ送信完了！" : "❌ 送信失敗",
      data: { target: nakaiInfo, messageType: customMessage ? "カスタムメッセージ" : "デフォルトメッセージ", deliveryStatus: result.success ? "配信成功" : "配信失敗", status: result.status, botName: "日向", apiType: "個別専用API" },
      timestamp: new Date().toISOString()
    });
  } catch (error) {
    return res.status(200).json({ success: false, error: error.message, target: "中井理恵さん", timestamp: new Date().toISOString() });
  }
}
